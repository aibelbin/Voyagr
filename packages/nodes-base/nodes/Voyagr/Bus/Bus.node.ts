...
export class Bus implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Bus', name: 'bus', icon: 'file:bus.svg', group: ['transform'], version: 1,
		description: 'A bus or coach journey', defaults: { name: 'Bus' },
		inputs: [NodeConnectionTypes.Main], outputs: [NodeConnectionTypes.Main],
		properties: [
			{ displayName: 'Operator', name: 'operator', type: 'string', default: '', placeholder: 'FlixBus' },
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		for (let i = 0; i < Math.max(items.length, 1); i++) out.push({ json: {
			kind: 'bus', operator: this.getNodeParameter('operator', i, '') as string, from: this.getNodeParameter('from', i, '') as string,
